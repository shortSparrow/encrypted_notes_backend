...
export const migrateFrom0To1 = async () => {
  ...
  await dbClient.query(
    `CREATE TABLE IF NOT EXISTS ${TableNames.DEVICES} (
      ...
      public_key_for_notes_encryption TEXT NOT NULL, 
  ...
  await dbClient.query(
    `CREATE TABLE IF NOT EXISTS ${TableNames.DEVICE_REFRESH_TOKENS} (
      id SERIAL PRIMARY KEY, 
      user_id SERIAL NOT NULL, 
  ...
  await dbClient.query(
    `CREATE TABLE IF NOT EXISTS ${TableNames.NOTES} (
      ...
      send_to_device_id TEXT NOT NULL, 
      note_global_id TEXT NOT NULL, 
      user_id SERIAL NOT NULL, 
      send_from_device_id TEXT NOT NULL,
      CONSTRAINT fk_notes FOREIGN KEY(send_to_device_id) REFERENCES ${TableNames.DEVICES}(device_id)
      ON DELETE CASCADE
    );`
  )
}
